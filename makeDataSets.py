...
if spark.catalog.tableExists("groupe5.players1"):
    fs.drop_table(
      name="groupe5.players1"
    )
...
if spark.catalog.tableExists("groupe5.players2"):
    fs.drop_table(
      name="groupe5.players2"
    )
...
playerwinCount = playerwinCount.drop("X").drop("winner").drop("tournament_key").drop("id").drop("best_of").drop("winner_id")
...
playerwinCara = spark.sql(
    '''
        ...
        SELECT p1_id, p2_id, p1.character AS c1, p2.character AS c2, location_names, CASE WHEN winner_id = p1_id THEN 'P1' ELSE 'P2' END AS win FROM groupe5.sets 
        INNER JOIN (
        SELECT winner_id AS winner, COUNT(winner_id) AS X FROM groupe5.sets GROUP BY winner_id HAVING x >= 1000
        ) x ON x.winner = sets.winner_id
...
playerwinCara.write.format("delta").mode("overwrite").option("userMetadata", "init").saveAsTable(f"groupe5.trainSet2")
