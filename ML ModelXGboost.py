...
# MAGIC This is an auto-generated notebook. To reproduce these results, attach this notebook to the **gtezenasdumontcel@myges.fr's Cluster** cluster and rerun it.
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/3036882950161435)
# MAGIC - Navigate to the parent notebook [here](#notebook/3036882950161436) (If you launched the AutoML experiment using the Experiments UI, this link isn't very useful.)
...
input_data_path = input_client.download_artifacts("136363c5939445f19865a74aff35c603", "data", input_temp_dir)
...
supported_cols = ["p2_id", "c1", "c2", "location_names", "p1_id"]
...
num_imputers = []
num_imputers.append(("impute_mean", SimpleImputer(), ["p1_id", "p2_id"]))
...
numerical_transformers = [("numerical", numerical_pipeline, ["p1_id", "p2_id"])]
...
categorical_one_hot_transformers = [("onehot", one_hot_pipeline, ["c1", "c2", "location_names"])]
...
# MAGIC 
# MAGIC `_automl_split_col_b721` contains the information of which set a given row belongs to.
...
# AutoML completed train - validation - test split internally and used _automl_split_col_b721 to specify the set
split_train_df = df_loaded.loc[df_loaded._automl_split_col_b721 == "train"]
split_val_df = df_loaded.loc[df_loaded._automl_split_col_b721 == "val"]
split_test_df = df_loaded.loc[df_loaded._automl_split_col_b721 == "test"]

# Separate target column from features and drop _automl_split_col_b721
X_train = split_train_df.drop([target_col, "_automl_split_col_b721"], axis=1)
y_train = split_train_df[target_col]

X_val = split_val_df.drop([target_col, "_automl_split_col_b721"], axis=1)
y_val = split_val_df[target_col]

X_test = split_test_df.drop([target_col, "_automl_split_col_b721"], axis=1)
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/3036882950161435)
...
def objective(params):
  with mlflow.start_run(experiment_id="3036882950161435", run_name="xgboost") as mlflow_run:
    ...
    xgbc_training_metrics = mlflow.sklearn.eval_and_log_metrics(model, X_train, y_train, prefix="training_", pos_label="P2")

    # Log metrics for the validation set
    xgbc_val_metrics = mlflow.sklearn.eval_and_log_metrics(model, X_val, y_val, prefix="val_", pos_label="P2")

    # Log metrics for the test set
    xgbc_test_metrics = mlflow.sklearn.eval_and_log_metrics(model, X_test, y_test, prefix="test_", pos_label="P2")
...
space = {
  "colsample_bytree": 0.7596615708035197,
  "learning_rate": 0.11574710322470175,
  "max_depth": 8,
  "min_child_weight": 6,
  "n_estimators": 1769,
  "n_jobs": 100,
  "subsample": 0.7902135477404391,
  "verbosity": 0,
  "random_state": 606161690,
}
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(100, X_train.shape[0]), random_state=606161690)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(100, X_val.shape[0]), random_state=606161690)
...
print(f"%md [Link to model run page](#mlflow/experiments/3036882950161435/runs/{ mlflow_run.info.run_id }/artifactPath/model)")
